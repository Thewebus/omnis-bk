<div class="col-4">
    @if (!is_null($dataNotes))
        ...
        <form wire:submit.prevent="postModificationSession2({{ $matiere->id }})" method="post">
            @csrf
                    ...
                    <tbody>
                        ...
                        @foreach ($dataNotes as $dataNote)
                            <tr>
                                ...
                                <td>
                                    <input class="form-control @error('partiel_session_2') is-invalid @enderror" wire:model="notes.{{ $dataNote['nom_etudiant']['id'] }}" type="number" step="0.01" placeholder="{{ $dataNote['partiel_session_2'] }}" />
            ...
            <div class="text-center">
                <button type="submit" class="btn btn-primary btn-block my-3 float-right">
                    <i class="fa fa-save"></i> Modifier les notes
                </button>
            </div>
        </form>
    @else

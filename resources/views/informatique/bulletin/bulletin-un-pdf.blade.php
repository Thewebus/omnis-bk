...
<body>
    ...
    @if ($anneeAcademique->id == 1)
        ...
        <div class="annee-session">
            ...
            <span class="numero-semestre">{{ $bulletinData['semestre'] == 1 ? '1er' : '2ème' }} Semestre - {{ $bulletinData['session'] == 1 ? '1ère' : '2ème' }} Session</span>
        </div>
        <div class="faculte" style="background-color: #1A0A8F; color:#fff; border-radius: 0.25rem;">
            <h3>{{ $bulletinData['faculte'] }}</h3> 
        </div>
        <div class="information">
            <div class="titre">
                <span class="id-bulletin">{{ $bulletinData['identifiant_bulletin'] }}</span>
                <span class="libelle">RELEVE DE NOTES</span>
            </div>
        ...
        <table class="ue-text">
            ...
            <tbody>
                    ...
                    @foreach ($uniteEnseignements as $uniteEnseignement)
                        ...
                        @else
                            <tr>
                                ...
                                <td class="text-center small" style="width: 3.5rem">{{ $uniteEnseignement['resultat'] }}</td>
                                <td class="text-center small" style="width: 3.5rem">{{ $uniteEnseignement['mention'] }}</td>
                                <td class="text-center small" style="width: 12rem">{{ $uniteEnseignement['nom_professeur'] }}</td>
                            </tr>
                        @endif
                    @endforeach
                @endforeach
                ...
                @endif --}}
                <tr>
                    <td style="background-color: rgb(186, 186, 186)" colspan="2">Crédit(s) Validé(s): {{ $bulletinData['total_credit_validee'] }} / {{ $bulletinData['total_credit'] }} </td>
                    <td style="background-color: rgb(186, 186, 186)" colspan="3"> Moyenne : {{ round($bulletinData['moyenne_finale'], 2) }} </td>
                    <td style="background-color: rgb(186, 186, 186)" colspan="2"> Résultat : {{ $bulletinData['resultat_final'] }} </td>
                </tr>
                @if ($bulletinData['semestre'] == 2)
                    ...
                    @endphp
                    <tr>
                        <td style="background-color: rgb(186, 186, 186)" colspan="2">Crédit(s) validé(s) Semestre 1 : {{ $bulletinData['credits_validee_semestre_1'] }} / {{ $bulletinData['total_credits_semestre_1'] }} </td>
                        <td style="background-color: rgb(186, 186, 186)" colspan="3">Moy. Semestre 1 : {{ round($bulletinData['moyenne_semestre_1'], 2) }} </td>
                        <td style="background-color: rgb(186, 186, 186)" colspan="2">Résultat Semestre 1 : {{ $bulletinData['resultat_semestre_1'] }}</td>
                    </tr>
                    <tr>
                        <td style="background-color: rgb(186, 186, 186)" colspan="2">Total Crédit(s) Validé(s) : {{ $total_credits_valides }}/{{ $total_credits }}  </td>
                        <td style="background-color: rgb(186, 186, 186)" colspan="3">Moy.Annuelle : {{ round($moyenne_annuelle, 2) }} </td>
                        <td style="background-color: rgb(186, 186, 186)" colspan="2">Décision du Jury : {{ $decision_jury }}</td>
                    </tr>
                @endif
                <tr>
                    {{-- <th style="padding-bottom: 10em" colspan="5">
                        @if ($bulletinData['semestre'] == 2)
                            <div>
                                <span class="underline"> Décision de fin d'année</span>
                            </div>
                        @endif
                    </th> --}}
    ...
    @else
        <div class="bloc-header">
            <div class="img">
                ...
                <div>{{ $bulletinData['identifiant_bulletin'] }}</div>
            </div>
            <div class="ministere">
                <span>REPUBLIQUE DE CÔTE D'IVOIRE <br> -------------------------- <br></span>
                <span>UNION - DISCIPLINE - TRAVAIL <br> ---------------------- <br></span>
        ...
        <table class="ue-text">
            ...
            <tr class="bg-grey {{ $bulletinData['semestre'] == 1 ? 'text-center' : '' }} align-middle">
                <td colspan="2">Crédit(s) Validé(s) : <span class="text-red">{{ $creditValides }} / {{ $bulletinData['total_credit'] }}</span></td>
